...
-   O rótulo deve começar com um caractere alfabético (de a até z ou de A até Z), e pode ser qualquer combinação de letras e números com até 16 caracteres. ** Observação: ** Um rótulo pode incluir caracteres o sublinhado (\_), mas nenhum outro caractere especial é permitido.
...
3.  Selecione um código de formato na lista. A tabela a seguir descreve os códigos de formato e suas ações.
    ...
    | CAL                           | Cálculo                        | Identifica uma linha que usa operadores matemáticos na coluna **Fórmulas/Linhas/Unidades Relacionadas**. Os cálculos contêm operadores complexos, como ** ** **+,-,\*** ** ** ** **,/, além ** **. instruções IF/THEN/ELSE |
...
No exemplo a seguir, Phyllis quer impedir a impressão do título e sublinhados da linha **Caixa Total** do seu relatório porque não houve atividade nas contas de caixa. Portanto, a linha 220 (que, como---** ** o código de formato indica, é uma linha de formatação,) ** fórmulas/linhas/unidades relacionadas ** na célula, entra ** ** 250, o código de linha de valor de linha que deseja suprimir. [![(RelatedRowsRowDefinition]. /media/relatedrowsrowdefinition-1024x144.png)](. /media/relatedrowsrowdefinition.png)
...
6.  No menu **Arquivo**, clique em **Salvar Como** para salvar a definição de coluna com um novo nome. Acrescentam ** CBR ** o nome de arquivo atual (por exemplo, ** CBR\_de YTD CUR\_**). Esta definição de coluna é a definição de coluna da linha base
...
No exemplo, os valores na coluna D do relatório serão classificadas em ordem crescente para as linhas 160 a 490. Além disso, os valores absolutos na coluna G do relatório serão classificadas em ordem decrescente para as linhas 610 a 940.
...
O tipo deve representar o valor positivo e o valor negativo. Normalmente, você insere um formato semelhante que diferencia valores positivos e negativos. Por exemplo, para especificar os valores positivos e negativos têm duas casas decimais, mas os valores negativos são exibidos entre parênteses, insira 0.00; **** (0.00). A tabela a seguir mostra a formatos personalizadas que podem ser usados para controlar o formato dos valores. Todos os exemplos começam pelo valor 1234.56.
...
5.  Em **Atributos**, siga estas etapas para adicionar uma entrada para cada atributo que deve ser incluído com o código de linha:
    1.  Clique duas vezes na célula **Atributo** e selecione um nome de atributo. ** Cuidado: ** Substituir o sinal numérico (\#) com um valor numérico.
...
Quando você seleciona uma conta específica, o designer de relatórios costuma combinar a conta e os anos fiscais, períodos e outras informações especificadas na definição de coluna. Você pode usar diferentes informações, como diferentes períodos fiscais, para linhas específicas. A tabela a seguir mostra os modificadores de conta que estão disponíveis. Substituir o sinal numérico (\#) por um valor que é igual ou menor que o número de períodos em um ano fiscal.

| Modificador da conta | O que é impresso                                                                           |
|------------------|-------------------------------------------------------------------------------------------|
| /BB              | O saldo inicial de uma conta.                                                     |
| /\#              | O saldo do período especificado.                                                     |
| /-\#             | O saldo do período que \# períodos antes do período atual.                  |
| /+\#             | O saldo do período que \# períodos após o período atual.                   |
| /C               | O saldo do período atual.                                                       |
| /C-\#            | O saldo do período que \# períodos antes do período atual.                  |
| /C +\#            | O saldo do período que \# períodos após o período atual.                   |
| /Y               | O saldo de ano até a data no período atual.                                      |
| /Y-\#            | O saldo anual durante o período que \# períodos antes do período atual. |
| /Y +\#            | O saldo anual durante o período que \# períodos após o período atual.  |
...
5.  ** ** Do campo, digite a dimensão inicial, ou clicar! [] Procurar (“https://i-technet.sec.s-msft.com/dynimg/IC679490.gif”) consulta de pesquisa de dimensões disponíveis. Para inserir um intervalo de dimensões, insira a dimensão final no campo **Até**.
...
Quando você inserir um valor de segmento natural ** ** dimensões na caixa de diálogo, é possível inserir um caractere curinga (? ou \*) em qualquer posição de um segmento. O designer de relatórios extrai todos os valores para as posições definidas sem considerar os caracteres curinga. Por exemplo, a definição de linha contém somente valores de segmento natural, e os segmentos naturais têm quatro caracteres. ** Inserir 6??? ** em seguida, você instrui o designer de relatórios para incluir todas as contas com um valor de segmento natural que inicie com um 6. Se você inseri-lo ** ** 6\*, os mesmos resultados são retornados, mas resulta também incluem valores de largura, variável como ** 60 ** ** ** e 600000. O designer de relatórios substitui cada caractere curinga (?) pelo intervalo completo dos valores possíveis, que incluem letras e caracteres especiais. Por exemplo, no intervalo de **12?0** até **12?4**, o caractere curinga em **12?0** é substituído pelo menor valor no conjunto de caracteres, e o caractere curinga em **12?4** é substituído pelo maior valor no conjunto de caracteres. **Observação:** Você deve evitar usar caracteres curinga para as contas inicial e final nos intervalos. Se você usar caracteres curinga na conta inicial ou final, poderá receber resultados inesperados.

### <a name="single-segment-or-single-dimension-ranges"></a>Intervalos de um único segmento ou de uma única dimensão

Você pode especificar um intervalo de valores de segmento ou de valores de dimensão. A vantagem de especificar um intervalo é não precisar atualizar a definição de linha sempre que um novo valor de segmento ou valor de dimensão é adicionado aos dados financeiros. Por exemplo, o intervalo do 6100:6900\]** +conta=\[recepções ** os valores das contas 6100 a 6900 em linha de valor. Quando um intervalo inclui um caractere curinga (?), o designer de relatórios não avalia o intervalo caractere por caractere. As extremidades inferior e superior do intervalo são determinadas. Depois, os valores finais e todos os valores entre eles são incluídos. ** Observação: ** O Report Designer não pode selecionar dimensões, contas, ou campos de sistema de erp Microsoft Dynamics que incluem alguns caracteres reservados a seguir: e, \*, \[, \], {}, ou. Você pode adicionar um E comercial (&) somente quando cria automaticamente definições de linha usando a caixa de diálogo **Inserir Linhas de Dimensões**.

### <a name="multiple-segment-or-multiple-dimension-ranges"></a>Intervalos de vários segmentos ou de várias dimensões

Quando você inserir um intervalo usando combinações de valores de várias dimensões, a comparação do intervalo será feita em A. \ \ dimensões financeiras base de dimensão por dimensão. A comparação de intervalo não pode ser feita caractere por caractere ou com base em segmento parcial. Por exemplo, o intervalo do 5000:6000\]** +conta=\[1000:2000,\]Departamento= do\[, center= de custo previsto\[00 **\]inclui apenas as contas que correspondem a cada segmento. Neste cenário, a primeira dimensão deve estar no intervalo de 5000 a 6000, a segunda dimensão deve estar no intervalo de 1000 até 2000, e a dimensão a última deve ser 00. Por exemplo, +conta= **\[5100\], departamento=\[1100,\]center= de custo previsto\[01 **\]não está incluído no relatório, porque o último segmento está fora do intervalo especificado. Se um valor de segmento conter espaços, coloque esse valor entre colchetes (\[ \]). Os valores a seguir são válidos para um segmento de quatro caracteres: **\[ 234\]\[123, \], \[1 34 **\]. Os valores de dimensão devem ser colocados entre colchetes\[ ( \]), e o criador de relatório adicionar esses colchetes para você. Quando um intervalo de vários segmentos ou de várias dimensões conter caracteres curingas (? ou \*), os limites alto e baixo de vários segmentos ou do intervalo inteiro de várias dimensões são determinados, e valores final e todos os valores entre eles serão incluídos. Se tiver um grande intervalo, como o intervalo de todas as contas de 40000 a 99999, procure especificar uma conta inicial e uma conta final válidas, sempre que possível. ** Observação: ** O Report Designer não pode selecionar dimensões, contas, ou campos de sistema de erp Microsoft Dynamics que incluem alguns caracteres reservados a seguir: e, \*, \[, \], {}, ou. Você pode adicionar um E comercial (&) somente quando cria automaticamente definições de linha usando a caixa de diálogo **Inserir Linhas de Dimensões**.
...
| Adicionar dois valores de segmento.                                                                 | +conta=\[\]1205 +conta=\[\]1210                                                                           |
| Adicionar valores de segmento que incluam caracteres curinga.                                    | +conta=\[120? +conta=\[11??\]                                                                             |
| Adicionar um intervalo de contas totalmente qualificadas                                                | +divisão= 000:100\]do\[, conta=\[1205\], departamento=\[00\]                                                   |
| Adicionar um intervalo de valores de segmento.                                                          | +conta= 1200:1205\]do\[                                                                                       |
| Adicionar um intervalo de valores de segmento que incluam caracteres curinga.                         | +conta=\[? 120: 130?\]                                                                                       |
...
| Subtrair um valor de segmento de outro valor de segmento.                                  | +conta=\[\]1205 - conta=\[\]1210                                                                           |
| Subtrair um valor de segmento que inclua um caractere curinga de outro valor de segmento. | +conta=\[\]1200 - conta=\[11??\]                                                                           |
| Subtrair um intervalo de contas totalmente qualificadas                                           | - Divisão= 000:100\]do\[1200:1205,\]do\[00:01 Conta=, Departamento=\]do\[                                           |
| Subtrair um intervalo de valores de segmento.                                                     | 1200:1205 Do\]- Conta=\[                                                                                       |
| Subtrair um intervalo de valores de segmento que incluam caracteres curinga.                    | - Conta=\[? 120: 130?\]                                                                                       |

Embora você possa mudar as contas diretamente, também pode usar a caixa de diálogo **Dimensões** para aplicar a formatação correta aos links de dados financeiros. Alguns valores pode incluir caracteres curinga (? ou \*). Entretanto, o Report Designer não pode selecionar dimensões, contas, ou campos de sistema de erp Microsoft Dynamics que incluem alguns caracteres reservados a seguir: e, \*, \[, \], {}, ou. **Observação:** Para subtrair valores, você deve colocar esses valores entre parênteses. Por exemplo, se você inserir 450? **- ** (4509), +conta= ** exibiu como\[4509\]- conta=\[450? **\], e você estão instruindo o designer de relatórios para subtrair o valor do segmento de conta 4509 do valor de qualquer segmento de conta que inicie com 450.
...
3.  Na primeira linha na caixa de diálogo **Dimensões**, siga estas etapas:
    ...
    2.  Clicar duas vezes ** operador +/- ** a célula, selecione o sinal de adição (** **+) ou (** **-) negativo o operador que se aplica a um ou vários valores ou conjuntos de dimensões na linha.
...
| Item                | Descrição                                                                                                                                                                                                                                                                                             |
|---------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Individual ou intervalo | ** ** Do campo, insira o nome de uma conta, ou clicar ** consulta ** o botão! Navegue [] https://i-technet.sec.s-msft.com/dynimg/IC679490.gif demanda (“”) para procurar a conta. Para selecionar um intervalo, insira ou procure um valor no campo **Até**.                                             |
| Conjunto de Valores de Dimensão | No campo **Nome**, insira o nome de um conjunto de valores de dimensão. Para criar, modificar, copiar ou excluir um conjunto, clique em **Gerenciar Conjuntos de Valores de Dimensão**. ** Fórmula ** o campo será preenchido com a fórmula ** vincular a dimensões financeiras ** de célula deste conjunto de valor de dimensão na definição de linha. |
