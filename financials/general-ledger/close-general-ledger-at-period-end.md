...
description: "Este tópico descreve as tarefas que geralmente são realizadas ao executar um fechamento de período para a contabilidade."
author: RobinARH
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
Este tópico descreve as tarefas que geralmente são realizadas ao executar um fechamento de período para a contabilidade. 

Na contabilidade, é possível concluir os procedimentos de fechamento por um período ou por um ano. Os processos de fechamento preparam o sistema para um novo período. Para preparar o sistema para um novo ano, você deve realizar o seguinte processo e anos. Cada empresa possui processos e etapas diferente que executa para o fechamento de um período. Veja a seguir algumas das etapas opcionais termina de período:
...
-   Defina períodos do razão para **Em espera**, de modo que não haja novos lançamentos. Também é possível restringir um período para um grupo de usuários específico durante as atividades de fim de período, para um melhor controle. Não é recomendável configurar os períodos fechados ** ** permanentemente, pois não é possível reabrir um período que foi fechado.

O espaço de trabalho financeiro de fechamento de período pode ser usado para organizar e controlar as tarefas necessárias para diversos processos de fechamento de período. Refira [o espaço de trabalho financeiro] do período (financial-period-close-workspace.md) [e final do ano seguinte] tópicos (Year-end-close.md) para obter mais informações. 
