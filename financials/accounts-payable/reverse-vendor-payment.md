...
Ocasionalmente, depois que um pagamento de fornecedor foi lançado, o pagamento deve ser estornado. O estorno é diferente da exclusão, da anulação, ou da rejeição de um pagamento. Você pode excluir um pagamento somente se o status for **Criado**. Este status indica que a liquidação foi criada mas ainda não foi gerado. Essa limitação sempre será aplicado, independentemente do método de pagamento. Você pode anular cheques não lançada depois geradas mas antes de lançadas. Se o pagamento gerado é realizado como uma transferência de fundo eletrônico (EFT), você pode rejeitar o pagamento antes de lançar. Para rejeitar um pagamento, altere ** status de pagamento ** o valor. Um pagamento que foi anulada ou rejeitado pode ser gerado novamente após ** status de pagamento ** o valor muda em nenhum. ** ** 

Depois que um pagamento é lançado, os estornos serão usadas. Os pagamentos efetuados eletronicamente não podem ser revertidos depois lançadas. Em vez disso, uma nova transação será criada para o valor de pagamento para a parte traseira de responsabilidade na conta de fornecedor. Há dois métodos para estornar cheques lançados. Em um método, os estornos são lançados imediatamente quando você clica em **Estorno de pagamento** na página **Cheque**. No outro método, quando você clicar em **Estorno de pagamento** na página **Cheque**, o estorno é enviado ao diário de estorno de cheque no Gerenciamento de caixa e bancos, onde um revisor poderá lançar ou rejeitar o estorno. 
...
> [!NOTE]
> Se você excluir o diário, a reversão será removido do sistema, mas o cheque original permanece ** cheque ** na página. O status do cheque não é mais **Cancelamento pendente**.
