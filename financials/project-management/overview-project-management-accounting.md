...
Um projeto é um grupo de atividades criada para fornecer um serviço, do roteiro, ou para obter um resultado. Projetos e recursos consomem geram resultados financeiros em forma receitas ou ativos.

## <a name="projects-across-industries"></a>Projetos em indústrias
A funcionalidade de gerenciamento de projetos e de contabilidade pode ser usada em várias indústrias, conforme mostrado na ilustração. [projetos![através indústrias (]. /media/projects-accross-industries.jpg)](. /media/projects-accross-industries.jpg) 

Em um call center, um bilhete pode ser usado para descrever o conjunto de ações que são necessárias para resolver uma chamada. As empresas de consultoria, como gerenciamento ou organizações ou agências publicitárias consultando técnicas, consulte as atividades de projetos. Em marketing, uma campanha representa um conjunto de trabalho que deve ser entregue. Na fabricação baseada em projeto, uma ordem de produção relaciona diversos trabalhos que devem ser feitos para gerar alguns bens acabados. O nome é usado deless, os projetos envolvem recursos, agendas, custo e, e a funcionalidade de gerenciamento do projeto e da contabilidade no Microsoft Dynamics 365 para operações pode ajudar no planejamento, a execução, e a análise desses projetos.
...
Durante o projeto de iniciação, diversos processos principais ocorrem. Você pode usar uma cotação de projeto para comunicar o trabalho, despesas, e materiais estimados para o cliente. Você pode registrar as condições, limites, os contratos de cobrança em um contrato de projeto. Você pode usar uma estrutura de detalhamento de trabalho para planejar e (WBS) estimar o trabalho. É possível configurar previsões e orçamentos para orientar o projeto. A ilustração a seguir mostra a estrutura de um projeto. [estrutura![projeto do![(]. /media/project-structure1.jpg)](. /media/project-structure1.jpg)  
...
Você pode criar seis tipos de projeto no Microsoft Dynamics 365 para as operações. Cada tipo de projeto é configurado de forma distinta para reconhecimento de custo e receita. A seleção de um tipo de projeto depende da finalidade do projeto. A tabela a seguir descreve o uso típico de cada tipo de projeto.
...
| Preço fixo       | Em projetos de preço fixo, as notas fiscais consistem em transações por conta. Um projeto de preço fixo é faturado de acordo com um plano de faturamento baseada em um contrato de projeto. A receita para um projeto de preço fixo pode ser calculada e lançada no projeto usando o método de porcentagem concluída. Como alternativa, a receita pode ser calculada e lançada quando o projeto for concluído, usando o método de contrato concluído. As empresas geralmente podem tirar proveito de usar o valor do trabalho em andamento (WIP) para calcular o nível de conclusão de um projeto ou um grupo de projetos.                                                                                                                                     |
| Investimento        | Projetos de investimento são os projetos que não geram ganhos imediatos. São usados nos projetos internos de longo prazo em que os custos devem estar capitalizados. Apenas o custo de itens, as horas e as despesas podem ser registrados em um projeto de investimento. Os custos em um projeto de investimento são rastreados e controlados com o recurso de previsão. Os projetos de investimento podem ser configurados com uma capitalização máxima opcional. Como um projeto de investimento em andamento, você registra os custos nas contas WIP, na qual os custos são mantidas até que o projeto foi preenchido. Quando o projeto é eliminado, transfere-se o valor do WIP a um ativo fixo, uma conta contábil ou a um novo projeto.
> [!NOTE] 
> As transações em projetos de investimento não são mostradas em ** lançar custos ** **, acumular receita ** **, ou criar propostas de nota fiscal ** na página.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 _=_ _=_ Custo o projeto      _=_ Como projetos de investimento, projetos de custo previsto são usados normalmente acompanhar projetos internos, e apenas as horas, despesas, e os itens podem ser registrados a eles. No entanto, os projetos de custo normalmente têm duração mais curta do que os projetos de investimento. Além disso, ao contrário dos projetos de investimento, os projetos de custo não podem ser capitalizados nas contas de balanço. Em vez disso, as transações do projeto são lançadas em contas de lucros e perdas. 
[!NOTE] 
> As transações em projetos de custo previsto não aparecerem em ** lançar custos ** **, acumular receita ** **, ou criar propostas de nota fiscal ** na página. Para projetos de custo previsto são usados normalmente acompanhar projetos internos, que geralmente não têm ser associados a uma conta de cliente. No entanto, se sua configuração exigir que sejam criadas requisições de itens para ordens de compra, será necessário associar o projeto de custo a um cliente. Essa associação é necessária porque as requisições de itens são gerenciadas como linhas da ordem de venda, e o sistema requer que um cliente seja especificado. No entanto, essa configuração não resultará na criação automática de requisições de itens de uma ordem de compra. Para projetos de custo, a configuração **Criar requisição de itens** é ignorada. Se for necessária uma requisição de itens em um projeto de custo, será possível criar uma manualmente, desde que um cliente esteja associado ao projeto. _=_ _=_ Interno          _=_ Os projetos internos são usadas a faixa de custo em um projeto interno é que a sua organização. Os projetos internos podem fornecer uma ferramenta de planejamento para gerenciar o consumo de recurso. ** Observação: ** As transações em projetos internos não aparecerem em ** acumula receita ou ** ** crie propostas de nota fiscal ** na página.                                                                                                                                                                                                                                                                       _=_ _=_ Hora              _=_ Os projetos de tempo são usados acompanhar os tempos associados atividades não passíveis e não produtivas, como um projeto doentes rastrear horas para funcionários. Os projetos das transações não são lançados no tempo até o razão. Em vez disso, é incluído nos relatórios de trabalho de utilização. Apenas as transações de hora podem ser registrados no tempo projetos. Use um diário ou uma folha de ponto de horas para registrar essas horas no projeto. Após as horas são registrados, eles aparecem como transações do projeto mas não têm correspondentes transações de comprovante.

> [!NOTE] 
> As transações em projetos de tempo não aparecerem em ** lançar custos ** **, acumular receita ** **, ou criar propostas de nota fiscal ** na página.                                                                                                                                                                                       |
...
Eis qualquer uma das maneiras em que você pode usar a funcionalidade de recurso:
...
-   Link de **** consumido/sem vínculo com a ordem – o **** vincular o projeto a um requisito de item. Com esse método, você pode ver os custos reais do projeto quando a produção estiver com o status de **Iniciado** ou for reportada como concluída. Os custos são lançados como várias transações de item de projeto para matérias-primas e horas consumidas pela produção. Você também pode definir o nível de hierarquia da BOM em que os projetos da produção são controlados.
...
> [!NOTE] 
> A guia de remessa para uma ordem de venda relativas não pode ser cancelado se os itens foram marcados já agrupamento. 
...
Microsoft Dynamics 365 para operações não diminui automaticamente previsões esperadas. Portanto, recomendamos que você mantenha um modelo de previsão original no projeto de preço fixo para estabelecer uma linha de base para comparação quando o projeto for concluído. 
> [!NOTE] quando você selecionar o método, usar pelo menos dois modelos de previsão. Um modelo deve conter a previsão original. Para o outro modelo, você deve copiar transações de previsão de outro modelo. Este método é válido somente para projetos de investimento e de preço fixo.</td>
> </tr>
<tr class="odd">
...
-   ** Data de faturamento: ** 31 de março de 2012. Esta data é determinada com base na frequência de nota fiscal. Neste exemplo, você pode definir a frequência da fatura para o mês atual. Isso significa que todas as transações lançadas no mês de março são faturadas no último dia do mês.
-   **: Data de vencimento** 14 de abril de 2012. Esta data é determinada com base nas condições de pagamento definidos para o projeto. Para este exemplo, você selecionou uma condições de pagamento de 14 dias. Sendo assim, 14 dias são adicionados à data da fatura para chegar em uma data de vencimento de 14 de abril de 2012.
-   ** Data de pagamento esperada de venda: ** 27 de abril de 2012. Essa data será calculada adicionando-se o número de dias ** dias de buffer geral do campo ** ** parâmetros de gerenciamento do projeto e explicar ** página no número de dias ** os dias de buffer ** de campo na página ** sobre contratos de projeto **, e adicione o total para o número de dias ** ** data de vencimento no campo. Neste exemplo, você inseriu **3** no campo **Dias do buffer geral** e **10** no campo **Dias do buffer individual**. Sendo assim, 13 dias são adicionados à data de vencimento para chegar a uma data de pagamento de vendas esperada de 27 de abril de 2012.
...
-   ** Data de pagamento esperada de venda: ** 29 de março de 2012. O cálculo incluirá três dias de buffer geral de 10 e os dias de buffer.
...
> [!NOTE] 
> A data de vencimento da ordem de compra será baseada na transação de fornecedor quando a ordem de compra do projeto é criada. A data de vencimento não é determinada por quaisquer configurações padrão. 
...
> [!NOTE] 
> Quando você usa ** controle de custo ** a página para exibir o status atual do projeto, use os modelos de previsão que foram selecionados para o original e o orçamento restante. Se você selecionar outros modelos de previsão durante o cálculo de custos, os resultados não serão exatos.
