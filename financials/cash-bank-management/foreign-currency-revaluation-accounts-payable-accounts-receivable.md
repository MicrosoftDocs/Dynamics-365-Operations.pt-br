...
Use ** reavaliação de moeda estrangeira ** o página abaixo ** tarefas periódicas ** para reavaliar transações abertas. Você pode executar o processo em tempo real ou agendá-lo para execução usando um lote. Quando você define as configurações para o processo de reavaliação, garanta verifique se deseja imprimir um relatório de resultados. O relatório de reavaliação não pode ser reimprimido depois que o processo seja concluída. Se você gerar o relatório de reavaliação de moeda estrangeira, ele mostrará vários saldos no nível do cliente/fornecedor e no nível da moeda:
...
Um registro é mantido sempre que você execute uma reavaliação de moeda estrangeira. No registro da página **Avaliação de moeda estrangeira**, selecione **Transações** para exibir a lista detalhada de transações criadas por causa da reavaliação. Cada transação de comprovante representa a transação aberta que foi reavaliada. Se uma transação aberta foi reavaliada mais de uma vez, verá duas registradoras que usam o mesmo comprovante. Um registro será para reversão de lucro não realizado ou perda anterior, e outro registro será para o novo lucro não realizado ou perdas. Para executar o processo de reavaliação, clique no botão **Reavaliação de moeda estrangeira**. Defina as configurações apropriadas para os seguintes parâmetros:
