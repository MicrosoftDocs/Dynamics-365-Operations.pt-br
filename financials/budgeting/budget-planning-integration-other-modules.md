---
title: "Integração do planejamento de orçamento a outros módulos"
description: 
author: twheeloc
manager: AnnBe
ms.date: 04/04/2017
...
# <a name="budget-planning-integration-with-other-modules"></a>Integração do planejamento de orçamento a outros módulos
...
-   ** Crie um novo plano de orçamento ** cria um novo plano com os atributos selecionado em ** destino ** seção. Esses atributos não precisam ser exclusivos. Portanto, dois planos podem ter o mesmo nome e outros valores.
...
Para todos os processos, ** origem ** guia permite filtrar dados com ** filtro ** o botão. Por padrão, os campos específicos são adicionados ao filtro para cada processo. Por exemplo, no processo **Gerar plano de orçamento da contabilidade**, as categorias **Conta contábil** e **Conta principal** estão disponíveis e aparecem na página de geração. Todos os campos que você adicionar ao filtro também serão adicionados à página, juntamente com os critérios adicionados.
...
O campo **Mínimo** permite que você defina o valor de limite para criar uma linha de plano de orçamento. Se o valor de origem for menor que esse número, a linha do plano de orçamento não será criada. Um valor de 0.00 ** ** reserva todos os valores exceto as linhas de limite os valores positivos. Nenhuma (linhas de limites de valor para os valores positivos. Os valores negativos são sempre incluídos e geralmente representam entradas de crédito.)
...
O processo **Gerar plano de orçamento a partir dos ativos fixos** não tem nenhuma opção para realizar a agregação por período ou dia. Também não houver nenhuma opção para definir como o plano histórico. Você pode usar o processo periódico para incluir projetou transações de ativos fixos no planejamento de orçamento.
...
As linhas no plano de orçamento são agregadas por conta contábil e a posição. Entretanto, poderá excluir o número de posição, para que as linhas sejam agregadas por conta contábil. apenas Na guia **Destino**, defina a opção **Incluir posição na linha do plano de orçamento** para **Não**.
