...
| Reserva de venda de item  | Determina se a reserva de itens é manual ou automática. Se a reserva é automática, o estoque é reservado quando as linhas de ordem são criadas. É possível fazer reservas no nível de número de item para BOMs (opção **Automática**) ou para os elementos individuais de uma BOM (opção **Explosão**). O valor padrão de reserva ** vendas de item pode ser herdado ** ** parâmetros de contas a receber. ** Nessa página, o valor é definido no campo reserva ** valores de vendas ** ** na seção ** ** ** na guia geral. |
