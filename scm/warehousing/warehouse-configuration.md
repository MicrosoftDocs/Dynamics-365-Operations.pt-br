...
Os limites de estoque de local ajuda a garantir que os trabalhos não são criados para solicitar que o estoque seja colocado em um local que não tenha físico a capacidade manter o estoque. Por exemplo, se uma localização em um depósito pode manter somente um palete por local, os limites de estoque de local podem ser habilitadas. ** Quantidade ** o valor pode ser definido ** **, 1 e unidade ** ** o valor pode ser definido PL ** ** dentro de um grupo de perfis específico de localização. 
...
Para criar rapidamente as localizações de um depósito, poderá usar ** configuração de localização ** o assistente. Como parte desse processo, você pode facilmente manter o formato dos nomes do local.
