---
title: "Faça ajustes manuais na previsão da linha de base"
...
# <a name="make-manual-adjustments-to-the-baseline-forecast"></a>Faça ajustes manuais na previsão da linha de base
...
-   A primeira coluna mostra os itens, as chaves de alocação de itens, as empresas etc. para os quais a previsão foi gerada. O subtítulo da página fornece uma descrição das dimensões de previsão atuais que aparecem na grade. Por exemplo, se o subtítulo de página serão ** empresa/site/chave de alocação de item **, e um dos cabeçalhos de linha na grade é USMF **/1/Alloc ** do\_, essa linha exibe a previsão para a empresa de USMF, 1, o site e do\_Alloc ** ** a chave de alocação de item.
...
## <a name="forecast-aggregation-and-deaggregation"></a>Agregação e deaggregation de previsão
...
Quando o ajuste da quantidade prevista é feito a nível agregado, o sistema a alocação tornada mais para distribuir as peso alteração entre as linhas que cria a agregação. 
...
-   Se você usar o dynamics 365 para o computador a Microsoft Azure de previsões de demanda de operações que sabe o serviço, você pode especificar a porcentagem do nível de confiança a previsão que é gerada necessário. Um intervalo de confiança consiste em um intervalo de valores que representam boas estimativas para a previsão de demanda. Um nível de confiança de 95% indica que há um risco de 5% de que a previsão de demanda fique fora do intervalo de confiança.
