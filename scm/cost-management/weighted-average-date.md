...
A versão atual usa o mesmo princípio de liquidação direto usado para a média ponderada em versões anteriores. O sistema liquida diretamente entre recebimentos e saídas. O sistema usa esse princípio de liquidação direta em situações específicas:
...
No cenário a seguir, foram lançados um recebimento e uma saída atualizados financeiramente. Durante o fechamento de estoque, o sistema como recebimento diretamente contra a saída, e nenhum ajuste para o preço de custo estimado é necessária na saída. A ilustração a seguir mostra estas transações:
...
-   Acima ou abaixo de cada seta vertical, o valor de transação de estoque é especificado em price* de *Unit@de *Quantity* o formulário.
...
A versão atual usa o mesmo princípio de liquidação direta para data de média ponderada usada em versões anteriores. O sistema liquida diretamente entre recebimentos e saídas. O sistema usa esse princípio de liquidação direta em situações específicas:

-   Um recebimento e uma ou mais saídas foram lançados no período.
-   Somente saídas foram lançadas no período e o estoque contém estoque disponível de um fechamento anterior.

Se selecionar ** incluir valor físico ** caixa de seleção para um item em ** o grupo modelo de item ** página, os recebimentos atualizados fisicamente de sistema usa quando calcular o custo estimado ou execução, a média. As saídas são lançadas com base nesse preço de custo estimado durante o período. Durante o fechamento de estoque, somente os recebimentos atualizados financeiramente serão considerados somente no cálculo de média ponderada.
