...
O sistema estima esse preço de custo médio para um item usando a seguinte fórmula: Preço estimado = (Valor físico + Valor financeiro) ÷ (Quantidade física + Quantidade financeira)
...
\* Numerador = (Valor físico + Valor financeiro) \*\* Denominador = (Quantidade física + Quantidade financeira) **Nota:** Se a opção **Incluir valor físico** não for selecionada para um item, o sistema usará 0 (zero) para o valor físico e para a quantidade física. Para obter informações sobre essa opção, consulte [Incluir valor físico](include-physical-value.md).
