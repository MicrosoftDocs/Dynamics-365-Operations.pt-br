...
Depois que uma ordem de produção for liberada, os trabalhos de produção relacionados à ordem ficarão visíveis e prontos para o registro. Operadores podem fazer registros de trabalho, como, parada, Início e término, ** terminal cartão de trabalho ** na página ou dispositivo ** cartão de trabalho ** a página. Tempo e a quantidade serão registradas automaticamente transferidos páginas de registro a diários de produção para controlar a trilha de tempo e da quantidade consumida.
...
O trabalho para a separação de matéria-prima é gerado durante a liberação. Os trabalhos são gerados apenas para a quantidade de material que foi fisicamente reservada para a ordem de produção antes que a ordem foi liberada. Esta configuração é necessária para produzir trabalho de depósito para a separação de matéria-prima:
