...
---
# <a name="improve-master-planning-performance"></a>Aumentar o desempenho de planejamento mestre

[!include [banner](../includes/preview-banner.md)]
[!include [banner](../includes/banner.md)]

Este tópico explica as várias opções que podem ajudar a melhorar o desempenho do planejamento mestre e a solução de problemas. Inclui informações sobre os parâmetros e definições e sobre configurações e ações recomendadas. Também contém um resumo de todos os parâmetros importantes que você deve considerar quando tiver trabalhos de planejamento mestre de execução longa.
...
## <a name="parameters-that-are-related-to-master-planning-performance"></a>Parâmetros que estão relacionados ao desempenho do planejamento mestre
...
O parâmetro **Número de threads** permite ajustar o processo de agendamento do planejamento mestre para ajudá-lo a ter um melhor desempenho no conjunto de dados específico. Especifica o número total de threads que serão usados para executar o planejamento mestre. Causa paralelização da execução do planejamento mestre e essa paralelização ajuda a reduzir o tempo de execução. 
...
> [!NOTE]
> Uma configuração de **0** (zero) para o parâmetro **Número de threads** aumenta o tempo de execução do planejamento mestre. Portanto, recomendamos que você sempre defina um valor maior que 0.

### <a name="number-of-tasks-in-helper-task-bundle"></a>Números de tarefas no pacote de tarefas do auxiliar

Alterando a configuração **Número de tarefas no pacote de tarefas** (isto é, o tamanho do pacote), você pode reduzir o tempo de execução. Essa configuração controla o número de itens planejados juntos por um único auxiliar.
...
Geralmente, recomendamos que você aumente o número de tarefas quando o número de itens for muito grande (em centenas de milhares). Caso contrário, será necessário diminuir o número de tarefas. Para os seguintes setores específicos, considere esses pontos:
...
O sistema pode coletar mais informações sobre as tarefas executadas durante o planejamento mestre. Para que o sistema colete estas informações, ative a configuração **Rastrear tempo de processamento** na caixa de diálogo **Execução do planejamento mestre**. As informações coletadas podem ajudá-lo a encontrar gargalos na execução. Por exemplo, quando o parâmetro **Números de tarefas no pacote de tarefas do auxiliar** for definido como **1**, você pode identificar o item que tem o maior tempo de execução. Também é possível comparar o tempo de execução de vários threads que têm um status de **Cobertura** e comparar a duração de cada tarefa.
...
Ao examinar o log da sessão, analise os seguintes pontos:
