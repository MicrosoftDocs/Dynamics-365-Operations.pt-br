...
**Exemplo 1 - Linha do plano de manutenção baseada no tempo:** Um trabalho de lubrificação pode ser configurado em um intervalo fixo, ocorrendo uma vez por semana. Para essa finalidade, selecione "Repetido a partir da data do plano" no campo **Tipo de intervalo**, conforme mostrado na figura abaixo.

![Figura 2](media/02-preventive-maintenance.png)

**Exemple 2 - Linha do plano de manutenção baseada no tempo:** Um trabalho de inspeção pode ser configurado para ser executado aproximadamente uma vez por semana. Para este objetivo, selecione "Repetido a partir da última ordem de serviço" no campo **Tipo de intervalo**, conforme mostrado na figura abaixo.

![Figura 3](media/03-preventive-maintenance.png)

**Exemplo 3 - Linha do plano de manutenção com base no contador:** Uma ilustração gráfica de um contador de hora para o qual uma nova linha de agendamento de manutenção é criada cada vez que passarem 250 horas. O tipo de intervalo desta linha com base no contador é "Repetido a partir da data de início". A data de início é a data de início dos ativos relacionados na exibição de detalhes **Todos os ativos** > Guia Rápida **Planos de manutenção do ativo** > campo **Data de início** ou na exibição de detalhes **Local funcional** Guia Rápida > **Planos de manutenção** > campo **Data de início**. Este é um exemplo de um plano de manutenção *preventiva* porque a linha do agendamento de manutenção é criada automaticamente cada vez que o limite (+ 250) for atingido. A figura abaixo mostra um exemplo gráfico.

![Figura 4](media/04-preventive-maintenance.png)


**Exemplo 4 - Linha do plano de manutenção baseada no contador:** Uma ilustração gráfica de uma diminuição no valor do contador, medindo o desgaste da pastilha de freio. Uma linha do agendamento de manutenção é criada quando um registro do contador abaixo de 20 mm é criado na pastilha de freio. O tipo de intervalo desta linha com base no contador é "Uma vez atingido abaixo" ou "Uma vez a partir da última data de início". Este é um exemplo de um plano de manutenção *reativo* porque a linha do agendamento de manutenção não é criada até que uma medição abaixo de 20 mm seja registrada. A figura abaixo mostra um exemplo gráfico.

![Figura 5](media/05-preventive-maintenance.png)


**Exemplo 5 - Linha do plano de manutenção com base no contador:** Uma ilustração gráfica de um contador com um limite de -18° Celsius. Uma linha do agendamento de manutenção é criada quando for feito um registro do contador acima de -18° Celsius. O tipo de intervalo desta linha com base no contador é "Uma vez atingido acima". Este é um exemplo de um plano de manutenção *reativo* porque a linha do agendamento de manutenção não é criada até que seja registrada uma medição maior que -18° Celsius. A figura abaixo mostra um exemplo gráfico.
