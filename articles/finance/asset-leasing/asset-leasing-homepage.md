---
title: Home page de arrendamento de ativos
description: Este tópico fornece uma visão geral da documentação de arrendamento de ativos e links para tópicos específicos.
author: moaamer
manager: Ann Beebe
ms.date: 10/27/2020
ms.topic: article
ms.prod: ''
ms.service: dynamics-ax-applications
ms.technology: ''
ms.search.form: ''
audience: Application User
ms.reviewer: roschlom
ms.custom: 4464
ms.assetid: 5f89daf1-acc2-4959-b48d-91542fb6bacb
ms.search.region: Global
ms.author: moaamer
ms.search.validFrom: 2020-10-27
ms.dyn365.ops.version: 10.0.14
ms.openlocfilehash: 805f57d8f7e1b3d5447d4c6934af3739a16cec16
ms.sourcegitcommit: eaf330dbee1db96c20d5ac479f007747bea079eb
ms.translationtype: HT
ms.contentlocale: pt-BR
ms.lasthandoff: 02/15/2021
ms.locfileid: "5256778"
---
# <a name="asset-leasing-home-page"></a>Home page de arrendamento de ativos

[!include [banner](../includes/banner.md)]

Este tópico lista os tópicos da ajuda que estão disponíveis para arrendamento de ativos e inclui links para tópicos específicos. 

O arrendamento de ativos é um recurso avançado para gerenciar, rastrear e automatizar transações financeiras do locatário para ativos arrendados no Microsoft Dynamics 365 Finance. O arrendamento de ativos está em conformidade com padrões internacionais de contabilidade (IFRS 16) e padrões de GAAP dos EUA (ASC 842). O arrendamento de ativos captura e processa informações sobre os arrendamentos e ajuda a gerar entradas de diário durante o ciclo de vida da concessão das entradas de reconhecimento inicial, de diário mensal, para deficiências e encerramento da concessão.  

## <a name="asset-leasing-topics"></a>Tópicos de arrendamento de ativos
O tópico a seguir está disponível para ajudar a configurar e usar o arrendamento de ativos. 

 - [Introdução ao arrendamento do ativo](asset-leasing-quick-start.md) - Este tópico descreve os recursos gerais de arrendamento de ativos e lista e define os termos usados no software e na documentação.
 
 ### <a name="set-up-asset-leasing"></a>Configurar arrendamento de ativos
 Esse grupo de tópicos pode ajudá-lo a configurar o arrendamento de ativos de uma forma ideal para a situação comercial de sua organização.  
  
  - [Configurar parâmetros de arrendamento](config-lease-parameters.md) 
  - [Configurar registros de arrendamento](set-up-lease-books.md)
  - [Criar grupo de arrendamento](create-lease-group.md)
  - [Configurar taxas indexadas](set-up-index-rate-types.md)
  - [Configurar tipos de despesa](set-up-expense-types.md)
  - [Configurar nomes de diário de arrendamento](set-up-lease-journal-names.md)
  - [Configurar contas de lançamento de arrendamento](set-up-lease-posting-accts.md)
  - [Atribuir sequências numéricas](leasing-number-sequences.md)
  - [Atribuir funções de usuário](lease-user-roles.md)

### <a name="manage-asset-leases"></a>Gerenciar arrendamentos de ativos
Este grupo de tópicos descreve o processo para adicionar arrendamentos, criar faturas de pagamento, ajustar concessões, importar informações de arrendamento e prejudicar um ativo de direito de uso. 

 - [Adicionar ou copiar um arrendamento](add-lease.md)
 - [Criar uma fatura de pagamento](create-payment-invoice.md)
 - [Registrar depreciação de ativo de direito de uso](record-rou-asset-depreciation.md)
 - [Reverter transações de arrendamento lançadas](reverse-posted-lease-trans.md)
 - [Ajustar um arrendamento](adjust-lease.md)
 - [Prejudicar um ativo de direito de uso](impair-rou-asset.md)
 - [Reverter transações de arrendamento lançadas](reverse-posted-lease-trans.md)
 - [Prejudicar um ativo de direito de uso](impair-rou-asset.md)
 - [Associar um arrendamento a um ativo fixo](associate-lease-with-fixed-asset.md)
 - [Registrar arrendamentos em moedas estrangeiras](record-leases-foreign-currency.md)
 - [Reavaliar pagamentos do arrendamento vinculados a uma taxa indexada](revalue-payments-tied-2-index-rate.md)
 - [Reclassificar parte de curto prazo de responsabilidade com arrendamento](reclassify-st-lease-liability.md)
 - [Exibir transações de passivos, ativos e despesas](view-asset-transactions.md)
 - [Funcionalidade com intervalo composto](compound-interval-functionality.md)
 - [Confirmar entradas de diário mensal em um lote](confirm-payment-schedules-in-batch.md)
 - [Criar entradas de diário mensal em um lote](create-monthly-journals-batch.md)
 - [Relatórios duplos](dual-reporting.md)
 - [Configurar um fluxo de trabalho de arrendamento](set-up-lease-wrkflw.md)
 - [Usar fluxos de trabalho de aprovação de arrendamento](use-create-lease-wrkflw.md)
 - [Gerenciar arrendamento por meio da estrutura de importação de arrendamento](manage-leases-thru-imprt-framewrk.md)
 
### <a name="asset-leasing-reporting"></a>Relatórios de arrendamento de ativo
Este tópico descreve os relatórios disponíveis para arrendamentos de ativo. 

 - [Relatórios de arrendamento de ativos](asset-leasing-rprts.md)
 

## <a name="additional-resources"></a>Recursos adicionais

### <a name="whats-new-and-in-development"></a>Novidades e o que está em desenvolvimento

Acesse os [planos de versão do Microsoft Dynamics 365](https://go.microsoft.com/fwlink/?linkid=2010158) para ver quais novos recursos foram planejados. 

### <a name="blogs"></a>Blogs

Você pode encontrar opiniões, notícias e outras informações no [blog do Microsoft Dynamics 365](https://community.dynamics.com/b/msftdynamicsblog?c=Enterprise) e no [blog de finanças do Microsoft Dynamics 365 Finance and Operations](https://community.dynamics.com/365/financeandoperations/b/financials).

O [Blog da Comunidade de Parceiros do Microsoft Dynamics Operations](https://community.dynamics.com/partner/b/operationspartnercommunityblog) fornece aos Parceiros do Microsoft Dynamics um recurso único para saberem mais sobre as novidades e as tendências do Dynamics 365.

### <a name="videos"></a>Vídeos

Confira os vídeos de instruções que agora estão disponíveis no [canal do Microsoft Dynamics 365 no YouTube](https://www.youtube.com/channel/UCJGCg4rB3QSs8y_1FquelBQ). 


[!INCLUDE[footer-include](../../includes/footer-banner.md)]