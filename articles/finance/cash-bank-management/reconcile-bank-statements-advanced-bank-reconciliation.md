...
description: O recurso Reconciliação bancária avançada permite que você importe extratos bancários eletrônicos e os reconcilie automaticamente com as transações bancárias do Microsoft Dynamics 365 Finance. Este artigo descreve o processo de reconciliação.
author: moaamer
ms.date: 06/22/2020
ms.topic: article
ms.prod: ''
...
Se existirem novas transações iniciadas pelo banco, como juros, taxas e tarifas, que ainda não estão no Finance, você poderá adicioná-las à um diário que esteja associado à reconciliação do extrato bancário selecionado. Selecione uma transação do extrato bancário na grade **Transações do extrato bancário** de transações não combinadas e, em seguida, clique em **Marcar como nova**. O status da transação é definido como **Nova**, e a transação será movida para a grade **Transações do extrato bancário** que representa as transações combinadas. Você irá lançar as transações marcadas como **Nova** posteriormente, pela página **Extrato bancário**. 
