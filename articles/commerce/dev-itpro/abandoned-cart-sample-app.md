...
O status do trabalho de recuperação do carrinho abandonado, as IDs do carrinho e os carimbos de data/hora modificados são armazenados no Azure Cosmos DB. Por padrão, as configurações no arquivo de configuração apontam para a instância do emulador local do Azure Cosmos DB. Ao implantar o conector na produção, é necessário atualizar essas configurações para que elas apontem para a instância do Azure Cosmos DB na sua assinatura do Azure. Para testes locais ou de área restrita, você pode usar o [Azure Cosmos Emulator](/azure/cosmos-db/local-emulator).
