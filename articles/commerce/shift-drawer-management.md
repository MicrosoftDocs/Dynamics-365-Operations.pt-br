...
O Commerce fornece várias opções de configuração e operações de PDV para oferecer suporte a uma grande variedade de processos comerciais de fechamento do dia para o PDV. Esta seção descreve alguns cenários típicos de turnos.
...
Para que os turnos compartilhados sejam usados em uma loja, a gaveta de dinheiro deve ser configurada como uma "gaveta de turno compartilhado" em **Retail e Commerce \> Configuração de canal \> Configuração de PDV \> Perfis de PDV \> Perfis de hardware \> Gaveta**. Além disso, os usuários devem ter uma ou ambas as permissões de turno compartilhado (Permitir gerenciar turno compartilhado e Permitir usar turno compartilhado).
...
Várias operações podem ser realizadas para alterar o estado de um turno, ou para aumentar ou diminuir a quantidade de dinheiro na gaveta. Esta seção descreve essas operações de turnos para o Modern POS e PDV de Nuvem.
...
Vá para **Retail e Commerce \> Canais \> Lojas \> Todas as lojas de varejo \> Demonstrativo/fechamento** e, para cada loja, defina os campos **Método do demonstrativo** e **Método de fechamento** como **Turno**.
