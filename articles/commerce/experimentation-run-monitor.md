...
Este tópico descreve como executar e monitorar seu experimento em um aplicativo de terceiros e alterar variações, se necessário. Antes de concluir as etapas deste tópico, você precisará [publicar](experimentation-preview-publish.md) seu experimento no Commerce. O diagrama a seguir mostra todas as etapas envolvidas na configuração e execução de um experimento em um site de comércio eletrônico no Dynamics 365 Commerce. Etapas adicionais são abordadas em tópicos separados.
...
1. Faça as alterações necessárias, visualize e publique as variações. Para obter mais informações, consulte [Visualizar e publicar um experimento](experimentation-preview-publish.md).
