...
1. No campo **Cliente padrão**, forneça um cliente padrão válido. Esses dados são usados para auxiliar no preenchimento automático de padrões quando registros de clientes são criados. Ao criar ordens de call center, não é aconselhável criar ordens para o cliente padrão.
