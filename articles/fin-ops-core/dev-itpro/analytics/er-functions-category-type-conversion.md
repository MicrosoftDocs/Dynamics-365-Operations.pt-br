...
description: Este artigo fornece informações sobre as funções de conversão que são compatíveis no relatório eletrônico (ER).
author: kfend
ms.date: 12/05/2019
ms.prod: ''
ms.technology: ''
...
As funções de conversão de tipo de relatório eletrônico (ER) podem ser usadas para converter valores entre tipos. Este artigo fornece um resumo dessas funções.
