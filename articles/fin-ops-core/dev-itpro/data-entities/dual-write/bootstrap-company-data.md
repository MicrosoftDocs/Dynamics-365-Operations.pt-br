...
---
# <a name="bootstrap-with-company-data-faq"></a><span data-ttu-id="98151-103">Perguntas frequentes sobre bootstrap com dados da empresa</span><span class="sxs-lookup"><span data-stu-id="98151-103">Bootstrap with company data FAQ</span></span>
 
[!include [banner](../../includes/banner.md)]


## <a name="why-do-i-need-bootstrapping"></a><span data-ttu-id="98151-104">Por que eu preciso fazer bootstrap?</span><span class="sxs-lookup"><span data-stu-id="98151-104">Why do I need bootstrapping?</span></span> 
<span data-ttu-id="98151-105">Talvez você tenha uma instância existente do Common Data Service ou de outro aplicativo do Dynamics 365 com dados comerciais e queira habilitar a conexão de gravação dupla nela.</span><span class="sxs-lookup"><span data-stu-id="98151-105">You might have an existing Common Data Service or other Dynamics 365 app instance with business data, and you want to enable dual-write connection against it.</span></span> <span data-ttu-id="98151-106">Nesse caso, você precisa fazer bootstrap de dados do Common Data Service ou de outros aplicativos do Dynamics 365 com informações da empresa antes de habilitar a conexão de gravação dupla.</span><span class="sxs-lookup"><span data-stu-id="98151-106">In this case, you need to bootstrap Common Data Service or other Dynamics 365 app data with company information before enabling dual-write connection.</span></span>  
 
## <a name="when-should-i-use-bootstrapping"></a><span data-ttu-id="98151-107">Quando devo usar bootstrap?</span><span class="sxs-lookup"><span data-stu-id="98151-107">When should I use bootstrapping?</span></span> 
<span data-ttu-id="98151-108">Você deve usar bootstrap antes de habilitar mapas de entidade de gravação dupla (durante a etapa 5).</span><span class="sxs-lookup"><span data-stu-id="98151-108">You should use bootstrapping before enabling dual-write entity maps (during step #5).</span></span>  
1. <span data-ttu-id="98151-109">Para configurar a conexão de gravação dupla entre instâncias de seu aplicativo do Finance and Operations e do Common Data Service ou de outro aplicativo do Dynamics 365, faça logon no aplicativo do Finance and Operations como administrador.</span><span class="sxs-lookup"><span data-stu-id="98151-109">To setup the dual-write connection between instances of your Finance and Operations app and the Common Data Service or other Dynamics 365 app, log in to the Finance and Operations app as an administrator.</span></span> 
2. <span data-ttu-id="98151-110">Vá para o módulo **Gerenciamento de Dados** e clique no botão **Gravação Dupla**.</span><span class="sxs-lookup"><span data-stu-id="98151-110">Go to the **Data Management** module and click the **Dual-Write** button.</span></span> <span data-ttu-id="98151-111">Isso abre o **Integrador de Dados**.</span><span class="sxs-lookup"><span data-stu-id="98151-111">This launches the **Data Integrator**.</span></span> 
3. <span data-ttu-id="98151-112">Crie a conexão de gravação dupla para uma ou mais empresas.</span><span class="sxs-lookup"><span data-stu-id="98151-112">Create the dual-write connection for one or more companies.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="98151-113">![Criar conexão de gravação dupla](media/dual-write-boot-1.png)</span><span class="sxs-lookup"><span data-stu-id="98151-113">![Create dual-write connection](media/dual-write-boot-1.png)</span></span>
4. <span data-ttu-id="98151-114">Habilite o mapa de entidade **Cdm_companies**.</span><span class="sxs-lookup"><span data-stu-id="98151-114">Enable the **Cdm_companies** entity map.</span></span> <span data-ttu-id="98151-115">Isso sincroniza as empresas do aplicativo do Finance and Operations com o Common Data Service.</span><span class="sxs-lookup"><span data-stu-id="98151-115">This synchronizes companies from the Finance and Operations app to Common Data Service.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="98151-116">![Habilitar o mapa de entidade](media/dual-write-boot-2.png)</span><span class="sxs-lookup"><span data-stu-id="98151-116">![Enable the entity map](media/dual-write-boot-2.png)</span></span>
5. <span data-ttu-id="98151-117">Execute o exemplo de código de bootstrap no Common Data Service ou em outra instância de aplicativo do Dynamics 365.</span><span class="sxs-lookup"><span data-stu-id="98151-117">Run the sample bootstrapping code on the Common Data Service or other Dynamics 365 app instance.</span></span>  
6. <span data-ttu-id="98151-118">Quando o bootstrap for concluído e o sistema estiver pronto para a sincronização, habilite os mapas de entidade.</span><span class="sxs-lookup"><span data-stu-id="98151-118">When the bootstrapping is done and the system is ready for the live sync, enable the entity maps.</span></span>  

    <span data-ttu-id="98151-119">Habilitar os mapas de entidade dispara a sincronização inicial de dados para os mapas de entidade habilitados.</span><span class="sxs-lookup"><span data-stu-id="98151-119">Enabling the entity maps triggers the initial data sync for the enabled entity maps.</span></span> <span data-ttu-id="98151-120">Os dados correspondentes às empresas escolhidas na conexão de gravação dupla são sincronizados entre o aplicativo do Finance and Operations e o Common Data Service.</span><span class="sxs-lookup"><span data-stu-id="98151-120">The data corresponding to the companies chosen on the dual-write connection is synchronized between the Finance and Operations app and Common Data Service.</span></span> 
 
## <a name="how-to-i-use-the-code-sample"></a><span data-ttu-id="98151-121">Como devo usar o exemplo de código?</span><span class="sxs-lookup"><span data-stu-id="98151-121">How to I use the code sample?</span></span>
<span data-ttu-id="98151-122">O exemplo de código é um aplicativo em C# que você pode carregar no Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="98151-122">The sample code is a C# application that you can load in Visual Studio.</span></span> <span data-ttu-id="98151-123">Ele usa as dependências de pacote NuGet no SDK do Common Data Service, que você pode atualizar por meio de ferramentas padrão do Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="98151-123">It takes NuGet package dependencies on the Common Data Service SDK, that you can refresh through standard Visual Studio tooling.</span></span> 

<span data-ttu-id="98151-124">Depois de descompactar e abrir a solução no Visual Studio e de restaurar os pacotes NuGet, procure **TODO** no código.</span><span class="sxs-lookup"><span data-stu-id="98151-124">After unzipping and opening the solution in Visual Studio and restoring the NuGet packages, search for **TODO** in the code.</span></span> <span data-ttu-id="98151-125">Cada decisão que você precisar tomar sobre como deseja fazer bootstrap das informações da empresa é registrada por um **TODO**, com um exemplo de código para implementação canônica.</span><span class="sxs-lookup"><span data-stu-id="98151-125">Each decision you need to make about how you want to bootstrap company information is noted by a **TODO**, with sample code for a canonical implementation.</span></span> 

<span data-ttu-id="98151-126">O exemplo de código mostra somente uma das várias maneiras pelas quais é possível categorizar registros de entidade por empresa.</span><span class="sxs-lookup"><span data-stu-id="98151-126">The sample code shows only one of many ways you might categorize entity records by company.</span></span> <span data-ttu-id="98151-127">Alterando a lógica nas seções **TODO**, você pode criar uma categorização personalizada.</span><span class="sxs-lookup"><span data-stu-id="98151-127">By changing the logic in the **TODO** sections, you can create your custom categorization.</span></span> 
 
## <a name="what-should-i-expect"></a><span data-ttu-id="98151-128">O que devo esperar?</span><span class="sxs-lookup"><span data-stu-id="98151-128">What should I expect?</span></span>
<span data-ttu-id="98151-129">Por padrão, o aplicativo de exemplo permite fornecer um dicionário de mapeamentos de código de unidade de negócios para empresa.</span><span class="sxs-lookup"><span data-stu-id="98151-129">By default, the sample application lets you provide a dictionary of business unit-to-company code mappings.</span></span> <span data-ttu-id="98151-130">Qualquer entidade da qual você fizer bootstrap com um campo **OwningBusinessUnit** será definida automaticamente para usar a empresa especificada.</span><span class="sxs-lookup"><span data-stu-id="98151-130">Any entity you bootstrap with an **OwningBusinessUnit** field is automatically set to use the specified company.</span></span> <span data-ttu-id="98151-131">Qualquer entidade sem um campo **OwningBusinessUnit**, como um produto, definirá a empresa com base no mapeamento com um valor de unidade de negócios vazio.</span><span class="sxs-lookup"><span data-stu-id="98151-131">Any entity without an **OwningBusinessUnit** field, such as product, will set the company based on the mapping with an empty business unit value.</span></span>

<span data-ttu-id="98151-132">O aplicativo de console espera um parâmetro, que pode ser **–simulate** ou **–apply**.</span><span class="sxs-lookup"><span data-stu-id="98151-132">The console application expects one parameter, either **–simulate** or **–apply**.</span></span> <span data-ttu-id="98151-133">Se você usar o parâmetro de linha de comando **–simulate**, nenhum dado será atualizado.</span><span class="sxs-lookup"><span data-stu-id="98151-133">If you use the **–simulate** command line parameter, then no data is updated.</span></span> <span data-ttu-id="98151-134">Somente arquivos **simulation_<entityname>.csv** são gerados no mesmo diretório da ferramenta, um para cada entidade que teria sido atualizada.</span><span class="sxs-lookup"><span data-stu-id="98151-134">Only **simulation_<entityname>.csv** files are generated in the same directory as the tool, one for each entity that would have been updated.</span></span> <span data-ttu-id="98151-135">Você pode examinar esses arquivos de modo iterativo e trabalhar para garantir que o código atualize os valores da empresa conforme esperado.</span><span class="sxs-lookup"><span data-stu-id="98151-135">You can iteratively review these files while working to ensure the code updates company values as expected.</span></span> 

<span data-ttu-id="98151-136">Quando terminar as atualizações simuladas, use o parâmetro **–apply**.</span><span class="sxs-lookup"><span data-stu-id="98151-136">When you finish with the simulated updates, then use the **–apply** parameter.</span></span> <span data-ttu-id="98151-137">Isso atualiza todos os registros que no momento possuem um valor de empresa incorreto, em lotes de 1000 registros de cada vez (por padrão).</span><span class="sxs-lookup"><span data-stu-id="98151-137">This updates all records that currently have an incorrect company value, in batches of 1000 records at a time (by default).</span></span> <span data-ttu-id="98151-138">O código é idempotente como fornecido, o que significa que você pode reexecutá-lo e somente as empresas atribuídas de maneira incorreta serão atualizadas.</span><span class="sxs-lookup"><span data-stu-id="98151-138">The code is idempotent as provided, meaning you can re-run it and only the incorrectly assigned companies will be updated.</span></span> <span data-ttu-id="98151-139">Ao executar com **–apply**, o código gera arquivos CSV das alterações feitas, que são nomeados como **applied_<entityname>.csv**.</span><span class="sxs-lookup"><span data-stu-id="98151-139">When running with **–apply**, the code outputs CSV files of the changes made, which are named **applied_<entityname>.csv**.</span></span> 
