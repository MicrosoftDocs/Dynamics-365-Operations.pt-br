...
description: Como fazer bootstrap de dados do Dataverse ou de outros aplicativos do Dynamics 365 com informações da empresa antes de habilitar a conexão de gravação dupla.
...
---
# <a name="bootstrap-with-company-data-faq"></a><span data-ttu-id="fbad9-103">Perguntas frequentes sobre bootstrap com dados da empresa</span><span class="sxs-lookup"><span data-stu-id="fbad9-103">Bootstrap with company data FAQ</span></span>
 
[!include [banner](../../includes/banner.md)]

[!include [rename-banner](~/includes/cc-data-platform-banner.md)]

## <a name="why-do-i-need-bootstrapping"></a><span data-ttu-id="fbad9-104">Por que eu preciso fazer bootstrap?</span><span class="sxs-lookup"><span data-stu-id="fbad9-104">Why do I need bootstrapping?</span></span> 
<span data-ttu-id="fbad9-105">Talvez você tenha uma instância existente do Dataverse ou de outro aplicativo do Dynamics 365 com dados comerciais e queira habilitar a conexão de gravação dupla nela.</span><span class="sxs-lookup"><span data-stu-id="fbad9-105">You might have an existing Dataverse or other Dynamics 365 app instance with business data, and you want to enable dual-write connection against it.</span></span> <span data-ttu-id="fbad9-106">Nesse caso, você precisa fazer bootstrap de dados do Dataverse ou de outros aplicativos do Dynamics 365 com informações da empresa antes de habilitar a conexão de gravação dupla.</span><span class="sxs-lookup"><span data-stu-id="fbad9-106">In this case, you need to bootstrap Dataverse or other Dynamics 365 app data with company information before enabling dual-write connection.</span></span>  
 
## <a name="when-should-i-use-bootstrapping"></a><span data-ttu-id="fbad9-107">Quando devo usar bootstrap?</span><span class="sxs-lookup"><span data-stu-id="fbad9-107">When should I use bootstrapping?</span></span> 
<span data-ttu-id="fbad9-108">Você deve usar bootstrap antes de ativar mapas da tabela de gravação dupla (durante a etapa 5).</span><span class="sxs-lookup"><span data-stu-id="fbad9-108">You should use bootstrapping before enabling dual-write table maps (during step #5).</span></span>  
1. <span data-ttu-id="fbad9-109">Para configurar a conexão de gravação dupla entre instâncias de seu aplicativo do Finance and Operations e do Dataverse ou de outro aplicativo do Dynamics 365, faça logon no aplicativo do Finance and Operations como administrador.</span><span class="sxs-lookup"><span data-stu-id="fbad9-109">To setup the dual-write connection between instances of your Finance and Operations app and the Dataverse or other Dynamics 365 app, log in to the Finance and Operations app as an administrator.</span></span> 
2. <span data-ttu-id="fbad9-110">Vá para o módulo **Gerenciamento de Dados** e clique no botão **Gravação Dupla**.</span><span class="sxs-lookup"><span data-stu-id="fbad9-110">Go to the **Data Management** module and click the **Dual-Write** button.</span></span> <span data-ttu-id="fbad9-111">Isso abre o **Integrador de Dados**.</span><span class="sxs-lookup"><span data-stu-id="fbad9-111">This launches the **Data Integrator**.</span></span> 
3. <span data-ttu-id="fbad9-112">Crie a conexão de gravação dupla para uma ou mais empresas.</span><span class="sxs-lookup"><span data-stu-id="fbad9-112">Create the dual-write connection for one or more companies.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="fbad9-113">![Criar conexão de gravação dupla](media/dual-write-boot-1.png)</span><span class="sxs-lookup"><span data-stu-id="fbad9-113">![Create dual-write connection](media/dual-write-boot-1.png)</span></span>
4. <span data-ttu-id="fbad9-114">Ativar o mapa da tabela **Cdm_companies**.</span><span class="sxs-lookup"><span data-stu-id="fbad9-114">Enable the **Cdm_companies** table map.</span></span> <span data-ttu-id="fbad9-115">Isso sincroniza as empresas do aplicativo do Finance and Operations com o Dataverse.</span><span class="sxs-lookup"><span data-stu-id="fbad9-115">This synchronizes companies from the Finance and Operations app to Dataverse.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="fbad9-116">![Habilitar o mapa de tabela](media/dual-write-boot-2.png)</span><span class="sxs-lookup"><span data-stu-id="fbad9-116">![Enable the table map](media/dual-write-boot-2.png)</span></span>
5. <span data-ttu-id="fbad9-117">Execute o exemplo de código de bootstrap no Dataverse ou em outra instância de aplicativo do Dynamics 365.</span><span class="sxs-lookup"><span data-stu-id="fbad9-117">Run the sample bootstrapping code on the Dataverse or other Dynamics 365 app instance.</span></span>  
6. <span data-ttu-id="fbad9-118">Quando o bootstrap for concluído e o sistema estiver pronto para a sincronização, habilite os mapas de tabela.</span><span class="sxs-lookup"><span data-stu-id="fbad9-118">When the bootstrapping is done and the system is ready for the live sync, enable the table maps.</span></span>  

    <span data-ttu-id="fbad9-119">Habilitar os mapas de tabela dispara a sincronização inicial de dados para os mapas de tabela.</span><span class="sxs-lookup"><span data-stu-id="fbad9-119">Enabling the table maps triggers the initial data sync for the enabled table maps.</span></span> <span data-ttu-id="fbad9-120">Os dados correspondentes às empresas escolhidas na conexão de gravação dupla são sincronizados entre o aplicativo do Finance and Operations e o Dataverse.</span><span class="sxs-lookup"><span data-stu-id="fbad9-120">The data corresponding to the companies chosen on the dual-write connection is synchronized between the Finance and Operations app and Dataverse.</span></span> 
 
## <a name="how-to-i-use-the-code-sample"></a><span data-ttu-id="fbad9-121">Como devo usar o exemplo de código?</span><span class="sxs-lookup"><span data-stu-id="fbad9-121">How to I use the code sample?</span></span>
<span data-ttu-id="fbad9-122">O exemplo de código é um aplicativo em C# que você pode carregar no Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="fbad9-122">The sample code is a C# application that you can load in Visual Studio.</span></span> <span data-ttu-id="fbad9-123">Ele usa as dependências de pacote NuGet no SDK do Dataverse, que você pode atualizar por meio de ferramentas padrão do Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="fbad9-123">It takes NuGet package dependencies on the Dataverse SDK, that you can refresh through standard Visual Studio tooling.</span></span> 

<span data-ttu-id="fbad9-124">Depois de descompactar e abrir a solução no Visual Studio e de restaurar os pacotes NuGet, procure **TODO** no código.</span><span class="sxs-lookup"><span data-stu-id="fbad9-124">After unzipping and opening the solution in Visual Studio and restoring the NuGet packages, search for **TODO** in the code.</span></span> <span data-ttu-id="fbad9-125">Cada decisão que você precisar tomar sobre como deseja fazer bootstrap das informações da empresa é registrada por um **TODO**, com um exemplo de código para implementação canônica.</span><span class="sxs-lookup"><span data-stu-id="fbad9-125">Each decision you need to make about how you want to bootstrap company information is noted by a **TODO**, with sample code for a canonical implementation.</span></span> 

<span data-ttu-id="fbad9-126">O exemplo de código mostra somente uma das várias maneiras pelas quais é possível categorizar linhas de entidade por empresa.</span><span class="sxs-lookup"><span data-stu-id="fbad9-126">The sample code shows only one of many ways you might categorize entity rows by company.</span></span> <span data-ttu-id="fbad9-127">Alterando a lógica nas seções **TODO**, você pode criar uma categorização personalizada.</span><span class="sxs-lookup"><span data-stu-id="fbad9-127">By changing the logic in the **TODO** sections, you can create your custom categorization.</span></span> 
 
## <a name="what-should-i-expect"></a><span data-ttu-id="fbad9-128">O que devo esperar?</span><span class="sxs-lookup"><span data-stu-id="fbad9-128">What should I expect?</span></span>
<span data-ttu-id="fbad9-129">Por padrão, o aplicativo de exemplo permite fornecer um dicionário de mapeamentos de código de unidade de negócios para empresa.</span><span class="sxs-lookup"><span data-stu-id="fbad9-129">By default, the sample application lets you provide a dictionary of business unit-to-company code mappings.</span></span> <span data-ttu-id="fbad9-130">Qualquer entidade da qual você fizer bootstrap com um campo **OwningBusinessUnit** será definida automaticamente para usar a empresa especificada.</span><span class="sxs-lookup"><span data-stu-id="fbad9-130">Any entity you bootstrap with an **OwningBusinessUnit** field is automatically set to use the specified company.</span></span> <span data-ttu-id="fbad9-131">Qualquer entidade sem um campo **OwningBusinessUnit**, como um produto, definirá a empresa com base no mapeamento com um valor de unidade de negócios vazio.</span><span class="sxs-lookup"><span data-stu-id="fbad9-131">Any entity without an **OwningBusinessUnit** field, such as product, will set the company based on the mapping with an empty business unit value.</span></span>

<span data-ttu-id="fbad9-132">O aplicativo de console espera um parâmetro, que pode ser **–simulate** ou **–apply**.</span><span class="sxs-lookup"><span data-stu-id="fbad9-132">The console application expects one parameter, either **–simulate** or **–apply**.</span></span> <span data-ttu-id="fbad9-133">Se você usar o parâmetro de linha de comando **–simulate**, nenhum dado será atualizado.</span><span class="sxs-lookup"><span data-stu-id="fbad9-133">If you use the **–simulate** command line parameter, then no data is updated.</span></span> <span data-ttu-id="fbad9-134">Somente arquivos **simulation_<entityname>.csv** são gerados no mesmo diretório da ferramenta, um para cada entidade que teria sido atualizada.</span><span class="sxs-lookup"><span data-stu-id="fbad9-134">Only **simulation_<entityname>.csv** files are generated in the same directory as the tool, one for each entity that would have been updated.</span></span> <span data-ttu-id="fbad9-135">Você pode examinar esses arquivos de modo iterativo e trabalhar para garantir que o código atualize os valores da empresa conforme esperado.</span><span class="sxs-lookup"><span data-stu-id="fbad9-135">You can iteratively review these files while working to ensure the code updates company values as expected.</span></span> 

<span data-ttu-id="fbad9-136">Quando terminar as atualizações simuladas, use o parâmetro **–apply**.</span><span class="sxs-lookup"><span data-stu-id="fbad9-136">When you finish with the simulated updates, then use the **–apply** parameter.</span></span> <span data-ttu-id="fbad9-137">Isso atualiza todas as linhas que no momento possuem um valor de empresa incorreto, em lotes de 1000 linhas de cada vez (por padrão).</span><span class="sxs-lookup"><span data-stu-id="fbad9-137">This updates all rows that currently have an incorrect company value, in batches of 1000 rows at a time (by default).</span></span> <span data-ttu-id="fbad9-138">O código é idempotente como fornecido, o que significa que você pode reexecutá-lo e somente as empresas atribuídas de maneira incorreta serão atualizadas.</span><span class="sxs-lookup"><span data-stu-id="fbad9-138">The code is idempotent as provided, meaning you can re-run it and only the incorrectly assigned companies will be updated.</span></span> <span data-ttu-id="fbad9-139">Ao executar com **–apply**, o código gera arquivos CSV das alterações feitas, que são nomeados como **applied_<entityname>.csv**.</span><span class="sxs-lookup"><span data-stu-id="fbad9-139">When running with **–apply**, the code outputs CSV files of the changes made, which are named **applied_<entityname>.csv**.</span></span> 
