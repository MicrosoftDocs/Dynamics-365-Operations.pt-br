...
Para ajudar você a entender como melhorar o gerenciamento de personalizações no nível organizacional, esta seção descreve como o gerenciamento de personalização funcionava antes das exibições serem disponibilizadas.
...
As exibições salvas podem ser usadas somente em ambientes da Camada 1 (Desenvolvimento/Teste) e da Camada 2 (Área restrita) para fazer testes adicionais e alterações de design. Uma visualização de Exibições salvas estará disponível em ambientes de produção em uma versão futura.
